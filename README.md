...
- **Python 3.12** - основной язык реализации пайплайна
- **Pandas / NumPy** - загрузка, очистка, агрегация и трансформация табличных данных
- **LightGBM** - градиентный бустинг для регрессии по каждому тикеру
- **scikit-learn** - кросс-валидация по времени (`TimeSeriesSplit`) и поиск гиперпараметров (`RandomizedSearchCV`)
- **OpenRouter API + openai/gpt-4o-mini** - извлечение структурированных признаков из текстов новостей
- **Asyncio / aiohttp** - асинхронная обработка новостей и работа с внешним API
- **Poetry** - управление зависимостями и воспроизводимым окружением
...
Архитектура реализована как последовательный batch-пайплайн из независимых модулей. Каждый этап формирует артефакт на диске, который становится входом для следующего этапа.

### 1) Ingestion Layer (загрузка исходных данных)
...
**Ответственность:**
- Чтение исходных данных свечей и новостей
- Базовая валидация обязательных колонок и форматов дат
- Приведение к единому формату для последующих шагов

### 2) NLP Feature Layer (обогащение новостей)

**Модуль:** `scripts/news_classification.py`  
**Вход:** `data/news.csv`  
**Выход:** `data/train_news_features.csv`

**Что происходит:**
- Для каждой новости вызывается LLM, который возвращает структурированные признаки:
  - `sentiment`
  - `importance`
  - `category`
  - `affected_tickers`
- Обработка выполняется асинхронно, чтобы ускорить массовый inference
- Реализованы retry-механизмы для устойчивости к временным ошибкам API

### 3) Feature Engineering Layer (сборка обучающего датасета)

**Модуль:** `scripts/dataset.py`  
**Вход:** `data/candles.csv` + `data/train_news_features.csv`  
**Выход:** `data/combined_dataset.csv`

**Что происходит:**
- На свечах вычисляются технические признаки:
  - лаговые значения (`LAGS`)
  - скользящие окна (`WINDOWS`)
- Новости агрегируются по тикеру и дате
- Категориальные признаки из новостей кодируются (one-hot encoding)
- Применяется временной сдвиг новостей на 1 день, чтобы не допускать утечки будущей информации

### 4) Modeling Layer (обучение и валидация)

**Модуль:** `scripts/train_models.py`  
**Вход:** `data/combined_dataset.csv`  
**Выход:** директория `ticker_models/` (модель + метаинформация по признакам на каждый тикер)

**Что происходит:**
- Данные разделяются с учетом временного порядка
- Для каждого тикера обучается отдельная модель LightGBM
- Подбор гиперпараметров выполняется через `RandomizedSearchCV`
- Оценка устойчивости модели идет через `TimeSeriesSplit`
- При наличии совместимого окружения используется GPU-ускорение

### 5) Inference Layer (прогноз и формирование результата)

**Модуль:** `scripts/make_submission.py`  
**Вход:** обученные модели из `ticker_models/` + признаки для периода прогноза  
**Выход:** `data/submission.csv`

**Что происходит:**
- Формируются будущие даты на горизонт `TARGET_DAYS`
- Подготавливается признаковое пространство в том же формате, что и на обучении
- Для каждого тикера подгружается соответствующая модель
- Предсказания собираются в единый submission-файл

### Оркестрация и поток данных

Полный поток данных:

`candles.csv + news.csv`  
`-> train_news_features.csv`  
`-> combined_dataset.csv`  
`-> ticker_models/*`  
`-> submission.csv`

Такой подход дает:
- **Прозрачность**: каждый шаг можно проверить отдельно
- **Повторяемость**: фиксированный порядок шагов и конфигурация в `config.py`
- **Расширяемость**: можно добавлять новые источники признаков без переписывания всего пайплайна
...
Результаты оценки моделей на тестовой выборке (20% последних данных):

| Тикер | MAE    | RMSE   | MAPE   |
|-------|--------|--------|--------|
| AFLT  | 0.0311 | 0.0412 | 0.05%  |
| ALRS  | 0.0302 | 0.0393 | 0.06%  |
| CHMF  | 0.2176 | 0.3083 | 0.02%  |
| GAZP  | 0.0480 | 0.0627 | 0.04%  |
| GMKN  | 0.0442 | 0.0591 | 0.04%  |
| LKOH  | 2.0118 | 2.7256 | 0.03%  |
| MAGN  | 0.0312 | 0.0399 | 0.08%  |
| MGNT  | 0.7028 | 0.9420 | 0.01%  |
| MOEX  | 0.0283 | 0.0367 | 0.01%  |
| MTSS  | 0.0318 | 0.0417 | 0.02%  |
| NVTK  | 0.2863 | 0.3960 | 0.03%  |
| PHOR  | 1.3232 | 1.8390 | 0.02%  |
| PLZL  | 0.2499 | 0.3406 | 0.02%  |
| ROSN  | 0.1785 | 0.2385 | 0.04%  |
| RUAL  | 0.0247 | 0.0325 | 0.07%  |
| SBER  | 0.1418 | 0.1820 | 0.05%  |
| SIBN  | 0.2257 | 0.2996 | 0.04%  |
| T     | 1.4584 | 1.9622 | 0.05%  |
| VTBR  | 0.3423 | 0.4938 | 0.42%  |
| **СРЕДНЕЕ** | **0.3899** | **0.5306** | **0.06%** |
...
Для оценки метрик используйте скрипт:
```bash
poetry run python scripts/evaluate_metrics.py
```
